...
export function SlidingLogoMarquee({
  ...
  // Ensure enough items to visually fill the marquee width
  const displayItems = useMemo(() => {
    if (!items || items.length === 0) return [] as SlidingLogoMarqueeItem[];

    // Estimate single item width from provided height prop and aspect ratio
    const heightPx = (() => {
    ...
    const itemHeight = heightPx * 0.8; // matches .sliding-marquee-item { height: 80% }
    const itemWidth = itemHeight * (16 / 9); // aspect-ratio: 16/9
    const gapPx = 8; // rough default gap since CSS uses gap variable; good enough

    const visibleCount = Math.max(1, Math.ceil((dimensions.width || 0) / (itemWidth + gapPx)));
    // Add buffer so items start from far right and continue smoothly
    const required = Math.max(visibleCount + 4, 8);
    const times = Math.ceil(required / items.length);

    const duplicated: SlidingLogoMarqueeItem[] = Array.from({ length: times })
      .flatMap((_, t) => items.map((it, idx) => ({ ...it, id: `${it.id}-${t}-${idx}` })));
    return duplicated;
  }, [items, dimensions.width, height]);
